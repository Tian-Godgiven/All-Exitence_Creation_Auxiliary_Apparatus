...
<script setup lang='ts'>
    ...
    const getItemX = (itemTime:number)=>{
        const x = getTimeLocation(itemTime,timeRule,startTime.value,10,minUnit.value) 
        return x
    }
    ...
    const startTime = computed<number>(()=>{
        ...
        const smallValue = getSmallestTime(minTime,timeRule,minUnit.value)
        //把它变成刻度的值
    ...
    provide("startTime",startTime.value)
    ...
    const timeLineLeft = ref(function(){
        ...
        if(!nowTime)return 0
        const x = getTimeLocation(nowTime,timeRule,startTime.value,tickSpacing,minUnit.value)
        if(!x)return 0
        return x + 10
    }()) //时间线当前的移动距离
