...
export type SettingProps<T,S> = {
	...
	//参与筛选函数的对象，必须是composed类型的值，若为空则会设置为composed(()=>target)，用于执行select函数的目标，执行过程只会传入其.value（这也是为什么必须要composed的原因
	selectTarget?:S,
...
export type SettingOption<T> = {
	...
	confrimValue?:(value:any)=>boolean  //设置项值确认条件，若为空则不做限制
